package raftpebble

import (
	"bytes"
	"os"
	"reflect"
	"testing"

	"github.com/cockroachdb/pebble"
	"github.com/hashicorp/raft"
)

// the directory created here needs to be cleaned up after it's used
func testPebbleStore(t testing.TB) *PebbleStore {
	dh, err := os.MkdirTemp("", "pebble")
	if err != nil {
		t.Fatal(err)
	}

	// Successfully creates and returns a store
	store, err := NewPebbleStore(dh)
	if err != nil {
		t.Fatal(err)
	}

	return store
}

func testRaftLog(idx uint64, data string) *raft.Log {
	return &raft.Log{
		Data:  []byte(data),
		Index: idx,
	}
}

func TestPebbleStore_Implements(t *testing.T) {
	var store interface{} = &PebbleStore{}
	if _, ok := store.(raft.StableStore); !ok {
		t.Fatalf("PebbleStore does not implement raft.StableStore")
	}
	if _, ok := store.(raft.LogStore); !ok {
		t.Fatalf("PebbleStore does not implement raft.LogStore")
	}
}

func TestPebbleOptionsReadOnly(t *testing.T) {
	dh, err := os.MkdirTemp("", "pebble")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(dh)

	store, err := NewPebbleStore(dh)
	if err != nil {
		t.Fatal(err)
	}

	// Create the log
	log := &raft.Log{
		Data: []byte("log1"),
		Index: 1,
	}

	// Attempt to store the log
	if err := store.StoreLog(log); err != nil {
		t.Fatal(err)
	}

	store.Close()
	options := Options{
		Path: dh,
		PebbleOptions: &pebble.Options{
			ReadOnly: true,
		},
	}

	roStore, err := New(options)
	if err != nil {
		t.Fatal(err)
	}
	defer roStore.Close()

	result := new(raft.Log)
	if err := roStore.GetLog(1, result); err != nil {
		t.Fatal(err)
	}

	// Ensure the log comes back the same
	if !reflect.DeepEqual(log, result) {
		t.Errorf("log and result aren't equal: %v, %v", log, result)
	}

	// Attempt to store the log, should fail on a read-only store
	err = roStore.StoreLog(log)
	if err != pebble.ErrReadOnly {
		t.Errorf("expected a read only error but got %v", err)
	}
}

func TestPebbleStore_FirstIndex(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// Should get 0 index on empty log
	idx, err := store.FirstIndex()
	if err != nil {
		t.Fatal(err)
	}
	if idx != 0 {
		t.Fatalf("expected idx to be 0 but it was %d", idx)
	}

	// Set a mock raft log
	logs := []*raft.Log{
		testRaftLog(1, "log1"),
		testRaftLog(2, "log2"),
		testRaftLog(3, "log3"),
	}
	if err := store.StoreLogs(logs); err != nil {
		t.Fatal(err)
	}

	// Fetch the first Raft index
	idx, err = store.FirstIndex()
	if err != nil {
		t.Fatal(err)
	}
	if idx != 1 {
		t.Fatalf("expected idx to be 1 but it was %d", idx)
	}
}

func TestPebbleStore_LastIndex(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// Should get 0 index on empty log
	idx, err := store.LastIndex()
	if err != nil {
		t.Fatal(err)
	}
	if idx != 0 {
		t.Fatalf("expected idx to be 0 but it was %d", idx)
	}

	// Set a mock raft log
	logs := []*raft.Log{
		testRaftLog(1, "log1"),
		testRaftLog(2, "log2"),
		testRaftLog(3, "log3"),
	}
	if err := store.StoreLogs(logs); err != nil {
		t.Fatal(err)
	}

	// Fetch the last Raft index
	idx, err = store.LastIndex()
	if err != nil {
		t.Fatal(err)
	}
	if idx != 3 {
		t.Fatalf("expected idx to be 3 but it was %d", idx)
	}
}

func TestPebbleStore_GetLog(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	log := new(raft.Log)

	// Should return an error on non-existent log
	if err := store.GetLog(1, log); err != raft.ErrLogNotFound {
		t.Fatalf("expected raft log not found error, got: %v", err)
	}

	// Set a mock raft log
	logs := []*raft.Log{
		testRaftLog(1, "log1"),
		testRaftLog(2, "log2"),
		testRaftLog(3, "log3"),
	}
	if err := store.StoreLogs(logs); err != nil {
		t.Fatal(err)
	}

	// Should return the proper log
	if err := store.GetLog(2, log); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(log, logs[1]) {
		t.Fatalf("bad: %#v", log)
	}
}

func TestBoltStore_SetLog(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// Create the log
	log := &raft.Log{
		Data:  []byte("log1"),
		Index: 1,
	}

	// Attempt to store the log
	if err := store.StoreLog(log); err != nil {
		t.Fatal(err)
	}

	// Retrieve the log again
	result := new(raft.Log)
	if err := store.GetLog(1, result); err != nil {
		t.Fatal(err)
	}

	// Ensure the log comes back the same
	if !reflect.DeepEqual(log, result) {
		t.Fatalf("bad: %v", result)
	}
}

func TestBoltStore_SetLogs(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// Create a set of logs
	logs := []*raft.Log{
		testRaftLog(1, "log1"),
		testRaftLog(2, "log2"),
	}

	// Attempt to store the logs
	if err := store.StoreLogs(logs); err != nil {
		t.Fatal(err)
	}

	// Ensure we stored them all
	result1, result2 := new(raft.Log), new(raft.Log)
	if err := store.GetLog(1, result1); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(logs[0], result1) {
		t.Fatalf("bad: %#v", result1)
	}
	if err := store.GetLog(2, result2); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(logs[1], result2) {
		t.Fatalf("bad: %#v", result2)
	}
}

func TestBoltStore_DeleteRange(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// Create a set of logs
	log1 := testRaftLog(1, "log1")
	log2 := testRaftLog(2, "log2")
	log3 := testRaftLog(3, "log3")
	logs := []*raft.Log{log1, log2, log3}

	// Attempt to store the logs
	if err := store.StoreLogs(logs); err != nil {
		t.Fatal(err)
	}

	// Attempt to delete a range of logs
	if err := store.DeleteRange(1, 2); err != nil {
		t.Fatal(err)
	}

	// Ensure the logs were deleted
	if err := store.GetLog(1, new(raft.Log)); err != raft.ErrLogNotFound {
		t.Fatalf("should have deleted log1")
	}
	if err := store.GetLog(2, new(raft.Log)); err != raft.ErrLogNotFound {
		t.Fatalf("should have deleted log2")
	}
}

func TestBoltStore_Set_Get(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// Returns error on non-existent key
	if _, err := store.Get([]byte("bad")); err != ErrKeyNotFound {
		t.Fatalf("expected not found error, got: %q", err)
	}

	k, v := []byte("hello"), []byte("world")

	// Try to set a k/v pair
	if err := store.Set(k, v); err != nil {
		t.Fatal(err)
	}

	// Try to read it back
	val, err := store.Get(k)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(val, v) {
		t.Fatalf("bad: %v", val)
	}
}

func TestBoltStore_SetUint64_GetUint64(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// Returns error on non-existent key
	if _, err := store.GetUint64([]byte("bad")); err != ErrKeyNotFound {
		t.Fatalf("expected not found error, got: %q", err)
	}

	k, v := []byte("abc"), uint64(123)

	// Attempt to set the k/v pair
	if err := store.SetUint64(k, v); err != nil {
		t.Fatal(err)
	}

	// Read back the value
	val, err := store.GetUint64(k)
	if err != nil {
		t.Fatal(err)
	}
	if val != v {
		t.Fatalf("bad: %v", val)
	}
}

// check that log values are separated from config values
func TestSeparationOfValues(t *testing.T) {
	store := testPebbleStore(t)
	defer store.Close()
	defer os.RemoveAll(store.path)

	// raft logs are stored with keys corresponding to their index numbers,
	// so we're going to manually set a value here with a key of 1. after
	// writing the raft logs, we should still be able to read this value out,
	// because we're using prefixes as pseudo buckets.
	if err := store.Set(uint64ToBytes(1), []byte("hi")); err != nil {
		t.Fatal(err)
	}

	// Set a mock raft log
	logs := []*raft.Log{
		testRaftLog(1, "log1"),
		testRaftLog(2, "log2"),
		testRaftLog(3, "log3"),
	}
	if err := store.StoreLogs(logs); err != nil {
		t.Fatal(err)
	}

	// Try to read our original entry back out
	val, err := store.Get(uint64ToBytes(1))
	if err != nil {
		t.Fatal(err)
	}

	if !bytes.Equal(val, []byte("hi")) {
		t.Fatalf("expected %v to equal %v but it didn't", val, []byte("hi"))
	}
}

func TestNewPebbleStore(t *testing.T) {
	dh, err := os.MkdirTemp("", "pebble")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(dh)

	// Successfully creates and returns a store
	store, err := NewPebbleStore(dh)
	if err != nil {
		t.Fatal(err)
	}

	// Ensure the file was created
	if store.path != dh {
		t.Fatalf("the store path (%q) doesn't match the given path (%q)", store.path, dh)
	}

	if _, err := os.Stat(dh); err != nil {
		t.Fatal(err)
	}

	// Close the store
	if err := store.Close(); err != nil {
		t.Fatal(err)
	}
}